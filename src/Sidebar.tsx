  ...
  return (
    <aside className="fixed left-0 top-0 h-screen flex flex-col py-8 w-72 rounded-r-[3rem] bg-[#800000] shadow-[40px_0_60px_-15px_rgba(42,47,50,0.15)] z-50 text-white border-r border-[#800000]/20" role="navigation" aria-label="Navigasi Sidebar Dashboard">
      ...
      <nav className="flex-1 overflow-y-auto" aria-label="Menu Utama">
        <Link to="/siswa" className={getLinkClass('home')} aria-label="Menu Home" aria-current={activeMenu === 'home' ? 'page' : undefined}>
          <span className="material-symbols-outlined" aria-hidden="true">home</span>
          <span>Home</span>
        </Link>
        <Link to="/iqra" className={getLinkClass('iqra')} aria-label="Menu Iqra" aria-current={activeMenu === 'iqra' ? 'page' : undefined}>
          <span className="material-symbols-outlined" aria-hidden="true">menu_book</span>
          <span>Iqra'</span>
        </Link>
        <Link to="/ayat-pendek" className={getLinkClass('short-verses')} aria-label="Menu Ayat Pendek" aria-current={activeMenu === 'short-verses' ? 'page' : undefined}>
      ...
      <div className="px-8 mt-auto pt-8 border-t border-white/10">
        <div className="flex items-center gap-3 mb-6 bg-white/10 p-3 rounded-2xl backdrop-blur-sm border border-white/5" aria-label="Profil pengguna: Ahmad, Level 2 Learner">
