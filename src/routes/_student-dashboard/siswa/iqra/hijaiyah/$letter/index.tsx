...
export const Route = createFileRoute('/_student-dashboard/siswa/iqra/hijaiyah/$letter/')({
...
function DetailHijaiyah() {
  ...
  const { letter } = useParams({ from: '/_student-dashboard/siswa/iqra/hijaiyah/$letter/' });
  ...
  useEffect(() => {
    ...
    if (condition === 'tuna_rungu_wicara') {
      navigate({ to: '/siswa/iqra/hijaiyah/$letter/isyarat', params: { letter } });
    }
  }, []);
  ...
  return (
    <div className="bg-[#f3f7fb] text-[#2a2f32] min-h-screen siswa-body flex flex-1">
      {/* Main Content Area */}
