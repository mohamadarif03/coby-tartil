...
export const Route = createFileRoute('/_student-dashboard/siswa/ayat-pendek/$surah/')({
  component: DetailAyat,
})

function DetailAyat() {
  const navigate = useNavigate();
  const { surah } = useParams({ from: '/_student-dashboard/siswa/ayat-pendek/$surah/' });
  useAccessibility('Detail Ayat'); // ADDED

  // Auto-redirect ke mode isyarat jika user tuna rungu/wicara
  useEffect(() => {
    const condition = localStorage.getItem('cobytartil_user_condition');
    if (condition === 'tuna_rungu_wicara') {
      navigate({ to: '/siswa/ayat-pendek/$surah/isyarat', params: { surah } });
    }
  }, []);
  ...
  return (
    <div className="bg-[#f3f7fb] text-[#2a2f32] siswa-body min-h-screen flex">
      {/* Main Content */}
      <main className="relative flex flex-col flex-1 w-full min-h-screen p-8 ml-0 overflow-hidden lg:ml-72 lg:p-12" id="main-content" role="main">
        ...
        <header className="flex items-center justify-between w-full mb-12">
          <div className="flex flex-col">
            <nav aria-label="Breadcrumb" className="text-sm font-medium text-[#575c60] flex items-center gap-2 mb-2">
              ...
              <a href="#" className="hover:text-[#800000] transition-colors" onClick={(e) => { e.preventDefault(); navigate({ to: '/siswa/ayat-pendek' }); }}>Ayat Pendek</a>
              <span className="text-xs material-symbols-outlined">chevron_right</span>
          ...
          <div className="relative grid items-start flex-1 grid-cols-1 gap-8 md:grid-cols-12">
            ...
            <div className="flex flex-wrap justify-center gap-6 mt-8 md:col-span-12">
              ...
              <button 
                ...
                onClick={() => navigate({ to: '/siswa/ayat-pendek/$surah/isyarat', params: { surah: 'al-kautsar' } })}
