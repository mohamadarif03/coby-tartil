...
export const Route = createFileRoute('/guru/monitoring/$studentId/')({
...
function RouteComponent() {
  const { studentId } = useParams({ from: '/guru/monitoring/$studentId/' })
  const student = useMemo(() => (StudentPlaceholders.find(i => String(i.id) === studentId)), [studentId])
  const navigate = useNavigate();

  useEffect(() => {
    if (!student) { navigate({to: '/guru/monitoring'}) }
  }, [student])
